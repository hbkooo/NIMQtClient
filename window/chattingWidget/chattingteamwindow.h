...
#ifndef NIMQTCLIENT_CHATTINGTEAMWINDOW_H
#define NIMQTCLIENT_CHATTINGTEAMWINDOW_H
 ...
 */
class ChattingTeamWindow : public QWidget {
Q_OBJECT

public:
    explicit ChattingTeamWindow(const nim::SessionData &data, QWidget *parent = nullptr);

    ~ChattingTeamWindow() override;
...
protected:
    ...
    signals:
    void updateChattingTeamWindowSignal();  // 获取完用户信息 userNameCard 之后发送信号更新聊天窗口的 header 控件。ChattingTeamWindow::updateChattingTeamWindow
    void updateMsgListWidgetSignal(int);
    void noMoreMessageSignal();         // 发送没有更多的聊天消息信息
    void closeChattingTeamWindowSignal(QString id);     // 关闭该聊天界面窗口
...
#endif //NIMQTCLIENT_CHATTINGTEAMWINDOW_H
