...
typedef void (*nim_client_reg_sync_multiport_push_config_cb)(const char* json_extension,
...
static void CallbackDisconnect(const char* json_res, const void* callback) {
    CallbackProxy::DoSafeCallback<Client::DisconnectCallback>(callback, [=](const Client::DisconnectCallback& cb) { CallbackProxy::Invoke(cb); });
}
...
void CallbackGetServerCurrentTime(int rescode, bool calc_local, uint64_t time, const void* callback) {
    CallbackProxy::DoSafeCallback<Client::GetCurrentServerTimeCallback>(
        callback, [=](const Client::GetCurrentServerTimeCallback& cb) { CallbackProxy::Invoke(cb, rescode, calc_local, time); }, true);
}
