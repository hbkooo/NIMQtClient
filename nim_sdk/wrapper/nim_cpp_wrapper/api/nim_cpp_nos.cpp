...
static void CallbackProgress(int64_t completed_size, int64_t total_size, const char* json_extension, const void* callback) {
    CallbackProxy::DoSafeCallback<NOS::ProgressCallback>(
        callback, [=](const NOS::ProgressCallback& cb) { CallbackProxy::Invoke(cb, completed_size, total_size); });
}
...
static void CallbackSpeed(int64_t speed, const char* json_extension, const void* callback) {
    CallbackProxy::DoSafeCallback<NOS::SpeedCallback>(callback, [=](const NOS::SpeedCallback& cb) { CallbackProxy::Invoke(cb, speed); });
}

static void CallbackTransferInfo(int64_t actual_size, int64_t speed, const char* json_extension, const void* callback) {
    CallbackProxy::DoSafeCallback<NOS::TransferInfoCallback>(
        callback, [=](const NOS::TransferInfoCallback& cb) { CallbackProxy::Invoke(cb, actual_size, speed); });
}
...
static void CallbackSafeURLToOriginURLResult(int res_code, const char* origin_url, const void* user_data) {
    CallbackProxy::DoSafeCallback<NOS::SafeURLToOriginURLCallback>(
        user_data, [=](const NOS::SafeURLToOriginURLCallback& cb) { CallbackProxy::Invoke(cb, (NIMResCode)res_code, PCharToString(origin_url)); },
        true);
}
