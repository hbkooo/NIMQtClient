...
#if defined(WIN32)
    get_proc_address_wrapper_ = [&](const std::string& function_name) -> void* { return ::GetProcAddress(instance_nim_, function_name.c_str()); };
#else
    get_proc_address_wrapper_ = [&](const std::string& function_name) -> void* { return dlsym(instance_nim_, function_name.c_str()); };
#endif
    return true;
}
