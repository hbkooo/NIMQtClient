...
/** @fn void nrtc_dev_enum_device_path(NRTCDeviceType type, const char *json_extension, nrtc_dev_enum_device_path_sync_cb_func cb, const void
 * *user_data) NRTC DEVICE 遍历设备
 ...
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_enum_device_path(NRTCDeviceType type,
                                                const char* json_extension,
                                                nrtc_dev_enum_device_path_sync_cb_func cb,
                                                const void* user_data);

/** @fn void nrtc_dev_start_device(NRTCDeviceType type, const char *device_path, unsigned fps, const char *json_extension,
 * nrtc_dev_start_device_cb_func cb, const void *user_data) NRTC DEVICE
 * 启动设备，同一NRTCDeviceType下设备将不重复启动，不同的设备会先关闭前一个设备开启新设备
 * @param[in] type NRTCDeviceType 见nrtc_device_def.h
 ...
 * @param[in] json_extension 打开摄像头是允许设置 kNRTCDeviceWidth 和
 * kNRTCDeviceHeight，并取最接近设置值得画面模式，kNRTCDeviceHeadphone标记音频播放设备会关闭回音消除功能
 * @param[in] cb 结果回调见nrtc_device_def.h
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_start_device(NRTCDeviceType type,
                                            const char* device_path,
                                            unsigned fps,
                                            const char* json_extension,
                                            nrtc_dev_start_device_cb_func cb,
                                            const void* user_data);
...
/** @fn void nrtc_dev_start_extend_camera(const char *id, const char *device_path, unsigned fps, const char *json_extension,
 * nrtc_dev_start_device_cb_func cb, const void *user_data) NRTC DEVICE
 * 启动辅助的摄像头，摄像头数据通过nrtc_dev_set_video_data_cb设置采集回调返回，不直接通过视频通话发送给对方，并且不参与设备监听检测
 ...
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_start_extend_camera(const char* id,
                                                   const char* device_path,
                                                   unsigned fps,
                                                   const char* json_extension,
                                                   nrtc_dev_start_device_cb_func cb,
 ...
 * NRTC DEVICE 监听音频数据扩展接口
 * @param[in] type 指定NRTCAudioDataCbType，监听伴音数据
 * @param[in] json_extension 参考NRTCAudioDataCbType的说明
 * @param[in] cb 结果回调见nrtc_device_def.h
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
 * @return void 无返回值
 */
...
/** @fn void nrtc_dev_set_audio_process_info(bool aec, bool ns, bool vad)
 ...
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_set_audio_process_info(bool aec, bool ns, bool vad);

/** @fn void nrtc_dev_preload_effect(int id, const char *path, const char *json_extension, nrtc_dev_audio_effect_opt_cb_func cb, const void
 * *user_data) NRTC DEVICE 预加载音效文件（空路径时为移除），音效个数限制5个，时长限制10秒；暂时只支持wav和mp3格式
 * @param[in] id 音效id，重复时覆盖
 * @param[in] path 音效文件的本地路径，如果填空为移除音效
 * @param[in] json_extension 扩展
 * @param[in] cb 结果回调见nrtc_device_def.h；异步回调操作结果
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_preload_effect(int id,
                                              const char* path,
                                              const char* json_extension,
                                              nrtc_dev_audio_effect_opt_cb_func cb,
                                              const void* user_data);

/** @fn void nrtc_dev_play_effect(int id, int loop_count, const char *json_extension, nrtc_dev_audio_effect_opt_cb_func cb, const void *user_data)
 * NRTC DEVICE 播放音效，由kNRTCDeviceTypeAudioOutChat播放
 * @param[in] id 音效id
 * @param[in] loop_count 播放次数，0无效，小于零无限循环
 * @param[in] json_extension 扩展
 * @param[in] cb 结果回调见nrtc_device_def.h；异步回调操作结果，成功则在播放结束后回调；当音效未加载或者前一次同一个id的音效未播放完成，则失败
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_play_effect(int id,
                                           int loop_count,
                                           const char* json_extension,
                                           nrtc_dev_audio_effect_opt_cb_func cb,
                                           const void* user_data);

/** @fn void nrtc_dev_stop_effect(int id, const char *json_extension, nrtc_dev_audio_effect_opt_cb_func cb, const void *user_data)
 * NRTC DEVICE 结束播放音效
 * @param[in] id 音效id
 * @param[in] json_extension 扩展
 * @param[in] cb 结果回调见nrtc_device_def.h；异步回调操作结果
 ...
 * @return void 无返回值
 */
NRTC_SDK_DLL_API void nrtc_dev_stop_effect(int id, const char* json_extension, nrtc_dev_audio_effect_opt_cb_func cb, const void* user_data);
