 ...
 * nim_sysmsg_receive_cb_func回调函数定义见nim_nos_def.h
 * @param[in] prg_user_data APP的自定义用户数据，SDK只负责传回给回调函数prg_cb，不做任何处理！
 * @return void 无返回值
 */
NIM_SDK_DLL_API void nim_talk_send_msg(const char* json_msg,
 ...
 * 注册接收批量消息(一个会话为单位)回调
 * （如果在注册了接收消息回调的同时也注册了该批量接口，当有批量消息时，会改走这个接口通知应用层，例如登录后接收到的离线消息等）
 * @param[in] json_extension json扩展参数（备用，目前不需要）
 * @param[in] cb		接收消息的回调函数, nim_talk_receive_cb_func回调函数定义见nim_talk_def.h
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
 * @return void 无返回值
 ...
 * @return void 无返回值
 */
NIM_SDK_DLL_API void nim_talk_reg_notification_filter_cb(const char* json_extension,
...
/** @fn char* nim_talk_create_retweet_msg(const char* src_msg_json,
 ...
 */
NIM_SDK_DLL_API char* nim_talk_create_retweet_msg(const char* src_msg_json,
                                                  const char* client_msg_id,
                                                  enum NIMSessionType retweet_to_session_type,
                                                  const char* retweet_to_session_id,
                                                  const char* msg_setting,
...
/** @fn void nim_talk_recall_msg(const char* json_msg,
                                 const char* notify,
                                 const char* json_extension,
 ...
 */
NIM_SDK_DLL_API void nim_talk_recall_msg(const char* json_msg,
                                         const char* notify,
                                         const char* json_extension,
                                         nim_talk_recall_msg_func cb,
                                         const void* user_data);

/** @fn void nim_talk_recall_msg2(const char* json_msg,
                                  const char* notify,
                                  const char* apnstext,
 ...
 * @param[in] pushpayload		第三方自定义的推送属性，限制json，长度2048
 * @param[in] json_extension json扩展参数（备用，目前不需要）
 * @param[in] cb		nim_talk_recall_msg_func回调函数定义见nim_talk_def.h
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
 * @return void 无返回值
 ...
 *				10508:本地错误码,超过配置有效期或者所需参数不存在
 */
NIM_SDK_DLL_API void nim_talk_recall_msg2(const char* json_msg,
                                          const char* notify,
                                          const char* apnstext,
                                          const char* pushpayload,
                                          const char* json_extension,
...
/** @fn void nim_talk_recall_msg_ex(const char* json_msg,
                                    const char* notify,
                                    nim_talk_recall_msg_func cb,
 ...
 *				10508:本地错误码,超过配置有效期或者所需参数不存在
 */
NIM_SDK_DLL_API void nim_talk_recall_msg_ex(const char* json_msg,
                                            const char* notify,
                                            nim_talk_recall_msg_func cb,
                                            nim_talk_recall_extra_params& extra_params,
                                            const void* user_data);
...
/** @fn void nim_talk_reply_msg(const char* json_msg,
 ...
 * @return void 无返回值
 */
NIM_SDK_DLL_API void nim_talk_reply_msg(const char* json_msg,
                                        const char* json_reply_msg,
                                        const char* json_extension,
                                        nim_nos_upload_prg_cb_func prg_cb,
                                        const void* prg_user_data);
