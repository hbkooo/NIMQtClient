...
#define kNERtcExternalVideoDeviceID     "rtc-video-external-device"   /**< 外部视频输入源设备ID，开启外部输入之后，需要通过setDevice设置此设备ID。*/
#define kNERtcAudioDeviceAutoID         "rtc-audio-device-auto"       /**< 音频设备自动选择ID，设置该ID为设备时，SDK会根据设备插拔系统设置等自动选择合适音频设备。*/
...
typedef enum
{
     ...
     * 
     * 在 iOS/Android 平台中：如果你使用前置摄像头，SDK 默认启用镜像模式；如果你使用后置摄像头，SDK 默认关闭镜像模式。
    */
...
struct NERtcAudioRecvStats
{
    ...
    int audio_loss_rate;    /**< 特定时间内的音频丢包率 (%)。*/   
    float volume;           /**< 音量，范围为 -127（最低）- 0（最高）。*/
};
...
#define kNERtcKeyExtraInfo                  "extra_info"   /**< string value. APP标识，用于后台识别用户产品名称 */
} // namespace" rtc
